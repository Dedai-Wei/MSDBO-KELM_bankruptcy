...
function [Best_Score, Best_Pos, CG_curve1] = NRBO(N, MaxIt, LB, UB, dim, fobj)
    %disp('调用NRBO优化PID……………………')
    % Input arguments:
    ...
    fitness = arrayfun(@(i) fobj(Position(i, :)), 1:N);
    ...
    %disp(['PID参数：',num2str(Best_Pos),'，评价指标：',num2str(Best_Score)])
