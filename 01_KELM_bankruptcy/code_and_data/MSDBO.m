...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fMin , bestX, Convergence_curve1 ] =MSDBO(N, iter,c,d,dim,fobj)
...
for t = 1 : iter    
     ...
     P_percent1=0.3;
   P_percent2=0.1;
   P_percent =P_percent2+(P_percent1-P_percent2)*t/iter;%创新1：自适应子种群分界策略
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    x( i , : ) =  pX(  i , :)+0.3*abs(pX(i , : )-worse)+a*0.1*(XX( i , :)); % Equation (1)
       ...
       x(i , : ) = newBound(x(i , : ), lb, ub,bestX,bestX2) ;%创新2：设计了一种新的边界控制公式来代替传统的边界控制方法，加速种群收敛。
       ...
       if rand<0.5
        ...
        x( i, : )=pX( i , : )+((randn(1)).*(pX( i , : )-Xnew11)+((rand(1,dim)).*(pX( i , : )-Xnew22)));
          ...
          w=wmin+rand()*(wmax-wmin);    
           w=abs(((2*rand()) - (1*rand()+rand()))/w^2);  
           
          x(i,:)=  rand*pX(i,:) +  w *(bestX-pX(i,:));%%创新3：设计增加一种新的觅食位置更新公式以加速算法收敛，以增加种群的变异概率，提高其跳出局部最优的能力。
