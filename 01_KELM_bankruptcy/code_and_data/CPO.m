...
% % 
function [Gb_Fit,Gb_Sol,Conv_curve1]=CPO(Pop_size,Tmax,lb,ub,dim,fobj)
   %disp('调用CPO优化PID……………………')
%%%%-------------------Definitions--------------------------%%
if max(size(lb)==1)
       lb=lb.*ones(1,dim);
...
N_min=15; %% Is the minimum population size.
...
fitness = arrayfun(@(i) fobj(X(i, :)), 1:Pop_size);
...
while t<=Tmax 
    ...
    %disp(['PID参数：',num2str(Gb_Sol),'，评价指标：',num2str(Gb_Fit)])

end
 Conv_curve1=[fitness1,Conv_curve ];
end